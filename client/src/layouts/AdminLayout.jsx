// layouts/AdminLayout.jsx — Updated with Complaints nav item
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row">
            ...
            <header className="md:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 sticky top-0 z-50">
                <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-white text-sm font-bold">Facility Mgmt</p></div>
                </div>
            ...
            {isMobileMenuOpen && (<div className="fixed inset-0 bg-black/60 z-40 md:hidden" onClick={closeMenu} />)}
            ...
            <aside className={`w-64 bg-slate-900/95 md:bg-slate-900/80 backdrop-blur-md border-r border-slate-800 flex flex-col fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    {navItems.map((item) => (
                        <NavLink key={item.to} to={item.to} end={item.end} onClick={closeMenu}
                            className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${isActive ? 'bg-blue-600/15 text-blue-400' : 'text-slate-400 hover:text-white hover:bg-slate-800/60'}`}>
                ...
                <div className="px-4 py-4 border-t border-slate-800 bg-slate-900 md:bg-transparent">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-xs text-slate-400 font-bold shrink-0">{user?.name?.[0] || 'A'}</div>
                        <div className="flex-1 min-w-0">
                            <p className="text-white text-sm font-medium truncate">{user?.name}</p>
                            <p className="text-slate-500 text-[10px] truncate">{user?.employeeCode}</p>
                        </div>
                    </div>
                    <button onClick={logout} className="w-full py-2 bg-slate-800/80 hover:bg-red-600/20 hover:text-red-400 text-slate-400 text-xs font-medium rounded-lg transition-all">Sign Out</button>
                </div>
            </aside>
